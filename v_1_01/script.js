class Game {
  ...
  generateBall() {
    this.ninjatween = new TimelineMax({ repeat: -1, paused: 1 });
    $('.scene .ninja-holder').append('<div class="ninja red" id="ninja"></div>');
    $('.scene .ninja-holder .ninja').append('<div id="ninja-g" style="width: 39px; height: 39px; position: absolute; border-radius: 50%; left: 14.3px;"></div>');
    $('.scene .ninja-holder .ninja').append('<div id="ninja-g" style="width: 24.5px; height: 24px; position: absolute;  border-radius: 50%; left: 19.5px; top: 30.5px;"></div>');
    $('.scene .ninja-holder .ninja').append('<div id="ninja-g" style="width: 8px; height: 8px; position: absolute;  border-radius: 50%; left: 9px; top: 10px;"></div>');
    $('.scene .ninja-holder .ninja').append('<div id="ninja-g" style="width: 6px; height: 6px; position: absolute;  border-radius: 50%; left: 10px; top: 18px;"></div>');
    $('.scene .ninja-holder .ninja').append('<div id="ninja-g" style="width: 10px; height: 18px; position: absolute;  border-radius: 50%; left: 17px; top: 45.5px; transform: rotate(52deg);"></div>');
    $('.scene .ninja-holder .ninja').append('<div id="ninja-g" style="width: 12px; height: 18px; position: absolute; border-radius: 50%; left: 37.5px; top: 44.5px; transform: rotate(-20deg);"></div>');
    $('.scene .ninja-holder .ninja').append('<img src="./img/n-stand.svg" style="position: absolute; width: 64px;height: 64px; z-index: 2;"></img>');                          
    this.bounce();
  }
  ...
  generateTweet() {
    ...
    window.open("https://twitter.com/intent/tweet?url=https://codepen.io/gregh/full/yVLOyO&amp;text=I scored " + this.score + " points on Ninja Hopper! Can you beat my score?&amp;via=fzs-lab&amp;hashtags=ninjahopper", "TweetWindow", "width=600px,height=300px,top=" + top + ",left=" + left);
  }
  ...
  intro() {
    ...
    let ninja = new TimelineMax({ repeat: -1, delay: 3 });
    introTl.
    ...
    staggerFromTo('.start-game .ninja-demo', 1, { scale: 0 }, { scale: 1, ease: Elastic.easeOut.config(1, 0.3) }, 0.8, 2);


    ninja.fromTo('.start-game .section-1 .ninja-demo', 0.5, { y: "0px" }, { y: "100px", scaleY: 1.1, transformOrigin: "bottom", ease: Power2.easeIn }).
    to('.start-game .section-1 .ninja-demo', 0.5, { y: "0px", scaleY: 1, transformOrigin: "bottom", ease: Power2.easeOut,
      onStart: () => {
        while (this.prevColor == this.color) {
          this.color = new Color().getRandomColor();
        }
        this.prevColor = this.color;
        var el = document.querySelectorAll('#ninja-d');

for (var i = 0; i < el.length; i++) {
    var currentEl = el[i];
    currentEl.style.backgroundColor = this.color;
}
      } });
  ...
  * Display score
  ...
  start() {
    ...
    // Clean up the stick and ninja holders
    // and generate new ones
    $('#sticks, .scene .ninja-holder').html('');
  ...
  stop() {
    ...
    $('#sticks, .scene .ninja-holder, #score').html('');
    TweenMax.killAll();
   ...
   * and the ninja appears and falls down
   */
  moveToStart() {
    ...
    TweenMax.fromTo('#ninja', this.time,
    {
      scale: 0 },
  ...
  rearrange() {
    ...
    this.ninjatween.timeScale(scale);
   ...
   * It checks if the ninja and stick colors match
   * And changes the ninja color
   */
  bounce() {

    this.ninjatween.
    to('#ninja', this.time / 2, { y: '+=250px', scaleY: 0.7, transformOrigin: "bottom", ease: Power2.easeIn,
      onComplete: () => {
        this.checkColor();
      } }).
    to('#ninja', this.time / 2, { y: '-=250px', scaleY: 1.1, transformOrigin: "bottom", ease: Power2.easeOut,
      onStart: () => {
        while (this.prevColor == this.color) {
          this.color = new Color().getRandomColor();
        }
        this.prevColor = this.color;
        var el = document.querySelectorAll('#ninja-g');

for (var i = 0; i < el.length; i++) {
    var currentEl = el[i];
    currentEl.style.backgroundColor = this.color;
} //asdf
        $('#ninja').removeClass('red').
        removeClass('yellow').
        removeClass('purple').
  ...
  checkColor() {
    ...
    let ninjaPos = $('#ninja').offset().left + $('#ninja').width() / 2;
    let stickWidth = $('.stick').width();
    let score = this.score;

    $('#sticks .stick').each(function () {
      if ($(this).offset().left < ninjaPos && $(this).offset().left > ninjaPos - stickWidth) {

        if (Color.getColorFromClass($(this)) == Color.getColorFromClass('#ninja')) {
...
class Color {
  ...
  /**
   * Since the ninja and sticks have several classes
...
class Animation {
  ...
  static generateSmallGlows(number) {
    ...
    for (let i = 0; i < number; i++) {
      ...
      star.className = i % 20 == 0 ? 'star star--big' : i % 9 == 0 ? 'star star--medium' : 'star';
      star.setAttribute('style', `top:${top}px;left:${left}px;animation-duration:${Math.round(Math.random() * 3000) + 3000}ms;animation-delay:${Math.round(Math.random() * 3000)}ms;`);
      $('.small-glows').prepend(star);
    }
    $('.small-glows').prepend('<div class="star comet"></div><div class="star comet-2"></div>');
  }
  /**
  ...
  static sceneAnimation() {
    ...
    var wind = TweenMax.to('.wind', speed * 3, { backgroundPositionX: '-=1001px', repeat: -1, ease: Power0.easeNone });
...
Animation.generateSmallGlows(100);
